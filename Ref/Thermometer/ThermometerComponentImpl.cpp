// ======================================================================
// \title  ThermometerComponentAc.cpp
// \author Auto-generated
// \brief  cpp file for Thermometer component base class
//
// \copyright
...
namespace Ref {
  ...
  namespace {
    ...
    typedef enum {
      THERMOMETER_COMPONENT_EXIT = Fw::ActiveComponentBase::ACTIVE_COMPONENT_EXIT,
      TEMPERATUREREQIN_TEMPERATUREREQ,
      CMD_THERMO_SET_TEMP
    } MsgTypeEnum;
    ...
    typedef union {
      BYTE port1[Ref::InputTemperatureReqPort::SERIALIZED_SIZE];
  ...
  Ref::InputTemperatureReqPort *ThermometerComponentBase ::
    get_temperatureReqIn_InputPort(NATIVE_INT_TYPE portNum)
  {
    FW_ASSERT(portNum < this->getNum_temperatureReqIn_InputPorts(),static_cast<AssertArg>(portNum));
    return &this->m_temperatureReqIn_InputPort[portNum];
  }

  Fw::InputCmdPort *ThermometerComponentBase ::
  ...
  void ThermometerComponentBase ::
    set_temperatureOut_OutputPort(
        NATIVE_INT_TYPE portNum,
        Ref::InputTemperaturePort* port
    )
  {
    FW_ASSERT(portNum < this->getNum_temperatureOut_OutputPorts(),static_cast<AssertArg>(portNum));
    this->m_temperatureOut_OutputPort[portNum].addCallPort(port);
  }
  ...
  {
    FW_ASSERT(portNum < this->getNum_temperatureOut_OutputPorts(),static_cast<AssertArg>(portNum));
    return this->m_temperatureOut_OutputPort[portNum].registerSerialPort(port);
  }
    ...
    regCommands(void)
  {
    ...
    this->m_CmdReg_OutputPort[0].invoke(
        this->getIdBase() + OPCODE_THERMO_SET_TEMP
    );
  }
  ...
  // ----------------------------------------------------------------------
    ...
    ThermometerComponentBase :: ThermometerComponentBase(const char* compName) :
    ...
    )
  {
    ...
    // Connect input port temperatureReqIn
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_temperatureReqIn_InputPorts();
        port++
    ) {

      this->m_temperatureReqIn_InputPort[port].init();
      this->m_temperatureReqIn_InputPort[port].addCallComp(
          this,
          m_p_temperatureReqIn_in
      );
      this->m_temperatureReqIn_InputPort[port].setPortNum(port);
      ...
      (void) snprintf(
          portName,
          sizeof(portName),
          "%s_temperatureReqIn_InputPort[%d]",
          this->m_objName,
          port
      );
      this->m_temperatureReqIn_InputPort[port].setObjName(portName);
#endif
    ...
    // Initialize output port temperatureOut
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_temperatureOut_OutputPorts();
        port++
    ) {
      this->m_temperatureOut_OutputPort[port].init();
      ...
      (void) snprintf(
          portName,
          sizeof(portName),
          "%s_temperatureOut_OutputPort[%d]",
          this->m_objName,
          port
      );
      this->m_temperatureOut_OutputPort[port].setObjName(portName);
#endif
  ...
  {
    FW_ASSERT(portNum < this->getNum_temperatureOut_OutputPorts(),static_cast<AssertArg>(portNum));
    this->m_temperatureOut_OutputPort[portNum].invoke(temperature);
  }
  ...
  NATIVE_INT_TYPE ThermometerComponentBase ::
    getNum_temperatureReqIn_InputPorts(void)
  {
    return (NATIVE_INT_TYPE) FW_NUM_ARRAY_ELEMENTS(
        this->m_temperatureReqIn_InputPort
    );
  }

  NATIVE_INT_TYPE ThermometerComponentBase ::
    getNum_temperatureOut_OutputPorts(void)
  {
    return (NATIVE_INT_TYPE) FW_NUM_ARRAY_ELEMENTS(
        this->m_temperatureOut_OutputPort
    );
  }
  ...
  bool ThermometerComponentBase ::
    isConnected_temperatureOut_OutputPort(NATIVE_INT_TYPE portNum)
  {
     FW_ASSERT(
        portNum < this->getNum_temperatureOut_OutputPorts(),
        static_cast<AssertArg>(portNum)
     );
     return this->m_temperatureOut_OutputPort[portNum].isConnected();
  }
    ...
    )
  {
    ...
    FW_ASSERT(callComp);
    ThermometerComponentBase* compPtr =
      (ThermometerComponentBase*) callComp;

    const U32 idBase = callComp->getIdBase();
    FW_ASSERT(opCode >= idBase, opCode, idBase);
    switch (opCode - idBase) {

      case OPCODE_THERMO_SET_TEMP: /*!<  */
        compPtr->THERMO_SET_TEMP_cmdHandlerBase(
            opCode,
            cmdSeq,
            args
        );
  ...
  void ThermometerComponentBase ::
    THERMO_SET_TEMP_cmdHandlerBase(
        FwOpcodeType opCode,
        U32 cmdSeq,
        Fw::CmdArgBuffer &args
    )
  {
    ...
    this->THERMO_SET_TEMP_preMsgHook(opCode,cmdSeq);
    ...
    // Serialize for IPC
    _status = msg.serialize((NATIVE_INT_TYPE)CMD_THERMO_SET_TEMP);
    FW_ASSERT (
    ...
    tlmWrite_THERMO_TEMP(F32 arg)
  {
    ...
    if (this->m_Tlm_OutputPort[0].isConnected()) {
      ...
      _id = this->getIdBase() + CHANNELID_THERMO_TEMP;
  ...
  Fw::Time ThermometerComponentBase ::
    getTime(void)
  {
    if (this->m_Time_OutputPort[0].isConnected()) {
      Fw::Time _time;
  ...
  void ThermometerComponentBase ::
    log_ACTIVITY_HI_TERMO_TEMP_UPDATED(
        F32 temperature
    )
  {
    ...
    _id = this->getIdBase() + EVENTID_TERMO_TEMP_UPDATED;
    ...
    if (this->m_LogText_OutputPort[0].isConnected()) {
      ...
      const char* _formatString =
        "(%s) %s: Temperature of the thermometer updated to: %f";
#else
      const char* _formatString =
        "%s: Temperature of the thermometer updated to: %f";
#endif
  ...
  void ThermometerComponentBase ::
    log_ACTIVITY_LO_TERMO_TEMP_REQ_RECEIVED(
        F32 request
    )
  {
    ...
    _id = this->getIdBase() + EVENTID_TERMO_TEMP_REQ_RECEIVED;
    ...
    if (this->m_Log_OutputPort[0].isConnected()) {
      ...
      _status = _logBuff.serialize(request);
      FW_ASSERT(
          _status == Fw::FW_SERIALIZE_OK,
          static_cast<AssertArg>(_status)
      );
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Thermometer received temperature request from ThermalControl: %f";
#else
      const char* _formatString =
        "%s: Thermometer received temperature request from ThermalControl: %f";
#endif
    ...
    )
  {
    ...
    ThermometerComponentBase* compPtr =
      (ThermometerComponentBase*) callComp;
    compPtr->temperatureReqIn_handlerBase(portNum, requestTempteratue);
  }
    ...
    )
  {
    ...
    _status = msg.serialize(
        static_cast<NATIVE_INT_TYPE>(TEMPERATUREREQIN_TEMPERATUREREQ)
    );
    FW_ASSERT (
        _status == Fw::FW_SERIALIZE_OK,
        static_cast<AssertArg>(_status)
    ...
    // Serialize argument requestTempteratue
    _status = msg.serialize(requestTempteratue);
    FW_ASSERT(
        _status == Fw::FW_SERIALIZE_OK,
        static_cast<AssertArg>(_status)
    );
  ...
  Fw::QueuedComponentBase::MsgDispatchStatus ThermometerComponentBase ::
    doDispatch(void)
  {
    ComponentIpcSerializableBuffer msg;
    ...
    if (msgType == THERMOMETER_COMPONENT_EXIT) {
    ...
    switch (msgType) {
      ...
      // Handle async input port temperatureReqIn
      case TEMPERATUREREQIN_TEMPERATUREREQ: {

        // Deserialize argument requestTempteratue
        F32 requestTempteratue;
        deserStatus = msg.deserialize(requestTempteratue);
        FW_ASSERT(
            deserStatus == Fw::FW_SERIALIZE_OK,
            static_cast<AssertArg>(deserStatus)
        );
      ...
      // Handle command THERMO_SET_TEMP
      case CMD_THERMO_SET_TEMP: {
        // Deserialize opcode
        FwOpcodeType opCode;
...
#endif
        // Call handler function
        this->THERMO_SET_TEMP_cmdHandler(
            opCode,
            cmdSeq,
            temperature
        );
